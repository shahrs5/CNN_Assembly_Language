...
# def reshape_and_write(results, output_path="output.txt", decimals=3):
#     """
...
#                     for i in range(r):
#                         row = block[i * c:(i + 1) * c]
#                         formatted = [f"{math.ceil(v * factor) / factor:.{decimals}f}" for v in row]
...
#     reshape_and_write(results, output_path="output.txt", decimals=3)
...
def reshape_and_write(results, output_path="output.txt", decimals=3, prob_decimals=2):
    ...
    with open(output_path, 'w') as out:
        for key, (r, c, d) in specs.items():
            ...
            if key == "probabilities":
                # Format probabilities to prob_decimals places
                formatted_probs = [f"{math.ceil(v * prob_factor) / prob_factor:.{prob_decimals}f}" for v in data]
                out.write(str(formatted_probs) + "\n")
            else:
                for depth_idx in range(d):
                    if key == "input_matrix":
                        out.write(f"Block 28×28:\n")
